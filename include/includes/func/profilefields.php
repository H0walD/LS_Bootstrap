...
function profilefields_change ($uid) {
  ...
  while ( $r = db_fetch_assoc($q)) {
    ...
    <label class="col-sm-2 control-label">'.$r['show'].'</label>
    <div class="col-sm-10">
      <input type="text" name="profilefields['.$r['id'].']" value="'.$r['val'].'" class="form-control" >
    </div>
  </div><br>';
  ...
  while ( $r = db_fetch_assoc($q)) {
    if ( $r['func'] == 1 ) {
      $str = @db_result ( db_query ("SELECT val FROM prefix_userfields WHERE uid = ".$uid." AND fid = ".$r['id']) , 0 );
			$l .= '<tr><td>'.$r['show'].'</td><td>'.$str.'</td></tr>';
    } elseif ( $r['func'] == 2 ) {
      $l .= '<tr><td colspan="2"><b>'.$r['show'].'</b></td></tr>';
    } elseif ( $r['func'] == 3 ) {
      $str = '';
      if (isset($a[$r['show']])) {
      ...
      if ( function_exists ( 'profilefields_show_spez_'.$r['show'] ) ) {
        ...
        $l .= '<tr><td>'.ucfirst($r['show']).'</td><td>'.$str.'</td></tr>';
      }
    }
  }
  return ( $l );
