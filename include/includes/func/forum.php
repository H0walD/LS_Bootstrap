...
function check_for_pm_popup () {
  ...
  if (1 == db_result(db_query("SELECT COUNT(*) FROM prefix_user where id = ".$_SESSION['authid']." AND opt_pm_popup = 1"),0,0) AND 1 <= db_result(db_query("SELECT COUNT(*) FROM prefix_pm WHERE gelesen = 0 AND status < 1 AND eid = ".$_SESSION['authid'] ),0) ) {
    ...
    //--></script>
    <div id="newPMdivID" style="position:absolute; top:80px;left:50%;margin-left:-125px;display:inline;width:250px;">
    <div class="well">
      <p style="font-size:0.875em;">
        <a style="float:right;" href="javascript:closeNewPMdivID()" rel="tooltip" title="Schließen"><i class="fa fa-times-circle fa-2x"></i></a><p>
        <p style="font-size:0.875em;"><b>Neue private Nachricht</b><br><br>
        Bitte deinen <a href="?forum-privmsg">Posteingang</a> kontrollieren! 
        Damit dieses Fenster dauerhaft verschwindet musst du alle neuen Nachrichten
        lesen, oder die Option in deinem <a href="?user-profil">Profil</a> abschalten.</p>
    </div>
    </div>
html;
    return ($x);
