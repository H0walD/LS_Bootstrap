...
case 'show' :
  if ($allgAr['gbook_koms_for_inserts'] == 1) {
    ...
    if ($r['page'] != '') {
      ...
      $r['page'] = '<a href="'.$r['page'].'" target="_blank">Webseite</a>';
		}
		if ($r['mail'] != '') {
	    $r['mail'] = '<a href="mailto:'.escape_email_to_show($r['mail']).'">Kontakt</a>';
		}
    ...
    while ($r1 = db_fetch_assoc($erg)) {
      ...
      if (has_right(-7, 'gbook')) { $r1['text'] .= '&nbsp;&nbsp;<a href="index.php?gbook-show-'.$id.'-d'.$r1['id'].'" rel="tooltip" title="'.$lang['delete'].'" /><i class="fa fa-trash-o"></i></a>'; }
      $tpl->set_ar_out($r1, 5);
      $i++;
    }
    $tpl->out(6);
...
default :
  ...
  $ein    = @db_result($ei1,0);
    ...
    $mail = '';
      ...
      $page = '<a href="'.$row->page.'" target="_blank">Webseite</a>';
		}
		if ($row->mail) {
	    $mail = '<a href="mailto:'.escape_email_to_show($row->mail).'">Kontakt</a>';
		}
    $koms = '';
    if ($allgAr['gbook_koms_for_inserts'] == 1) {
