...
function icUpload () {
      ...
      if (empty($name)) {
        return ('<div class="alert alert-danger">keinen Namen angegeben.</div>');
      }

      if (empty($desc) or empty($descl)) {
        return ('<div class="alert alert-danger">kein langer oder/und kein kurzer Text angegeben.</div>');
      }

      if (empty($url) AND empty($_FILES['file']['name'])) {
        return ('<div class="alert alert-danger">Keine Datei oder Link angegeben.</div>');
      }
        ...
        $fende = strtolower($fende);
          ...
          return ('<div class="alert alert-info">Die Datei darf NICHT gr&ouml;sser als 2 MBytes sein.</div>');
        }
        ...
        if (
          ...
          return ('<div class="alert alert-info">Die Datei darf nur die Endungen: .zip, .tar oder .rar haben.</div>');
        }
        ...
        if (file_exists( 'include/downs/downloads/user_upload/'.$fname ) ) {
          return ('<div class="alert alert-info">Die Datei existiert bereits und kann nicht &uuml;berschrieben werden.</div>');
        }
...
switch ( $menu->get(1) ) {
  default :
    ...
    if (@db_num_rows($erg) <> 1) {
      ...
      $design = new design ( $title , $hmenu );
      $design->header();
      echo '<div class="alert alert-danger">Der Download wurde nicht gefunden</div>';
      $design->footer(1);
    }
  ...
  case 'upload' :
    if ( $allgAr['archiv_down_userupload'] == 1 AND loggedin() AND is_writeable ( 'include/downs/downloads/user_upload' ) ) {
      ...
      $re = icUpload();
	    if ($re === true) {
        echo '<div class="alert alert-success">Erfolgreich eingetragen! ... ein Moderator oder Admin dieser Seite wird den Download in n&auml;chster Zeit freischalten.</div>';
      } else {
