...
function news_find_kat ($kat) {
		...
		if ( !empty( $pfadzumBild ) ) {
			$kategorie = '<img  class="media-object hidden-xsmin" src="'.$pfadzumBild.'" alt="'.$kat.'">';
		} else {
  ...
  else
  {
    ...
    while ($row = db_fetch_assoc($erg)) {
      ...
      if ( strpos ( $row['text'] , '[PREVIEWENDE]' ) !== FALSE ) {
        $a = explode('[PREVIEWENDE]' , $row['text']);
        $row['text'] = $a[0];
        $row['readwholenews'] = '<a class="btn btn-primary btn-xs" href="index.php?news-'.$row['id'].'">'.$lang['readwholenews'].'</a>';
      } else {
        $row['readwholenews'] = '';
      }
