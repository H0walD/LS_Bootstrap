...
if ((isset($_POST['del']) OR isset($_POST['shift']) OR isset($_POST['status'])) AND (empty($_POST['in']) OR (isset($_POST['in']) AND (!is_array($_POST['in']) OR count($_POST['in']) < 1)))) {
  ...
  if (isset($_POST['del'])) {
    ...
    echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><p>Sicher die ausgewahlten Themen loeschen?</p><p><input type="submit" class="btn btn-danger" value="'.$lang['yes'].'" name="del" /></p></div>';
  } elseif (isset($_POST['shift']) AND $_POST['nfid'] != 'cat') {
    echo '<input type="hidden" name="afid" value="'.$fid.'"><legend>neues Forum ausw&auml;hlen</legend><br>';
    echo '<select name="nfid">';
      ...
      forum_admin_selectcats(0,0,$fid);
		echo '</select><br /><input type="checkbox" name="alertautor" value="yes" /> Die Autoren &uuml;ber das verschieben informieren?<br /><input class="btn btn-primary btn-xs" type="submit" value="'.$lang['shift'].'" name="shift">';
  }
