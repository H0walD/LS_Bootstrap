...
$hmenu  = $extented_forum_menu.'<a href="index.php?forum">Forum</a><b> &raquo; </b>'.aktForumCats($aktForumRow['kat']).'<b> &raquo; </b><a href="index.php?forum-showtopics-'.$fid.'">'.$aktForumRow['name'].'</a><b> &raquo; </b>';
...
if (($_SESSION['klicktime'] + 15) > $dppk_time OR empty($txt) OR !empty($_POST['priview']) OR (empty($_POST['Gname']) AND !loggedin()) OR !chk_antispam ('newpost')) {
  ...
  if ( !loggedin() ) {
    ...
    <label for="eingabefeldfname" class="col-sm-2 control-label">'.$lang['name'].'</label>';
    $name .= '<div class="col-sm-10">
      <input type="text" value="'.unescape($xnn).'" maxlength="15" class="form-control" name="Gname" placeholder="Dein Name">
    </div>
  </div>';
  }
