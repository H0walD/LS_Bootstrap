...
switch($uum) {
  case 1 : # change topic title
	...
	case 2 : # delete topic
	  if (empty($_POST['sub'])) {
      echo '<form action="index.php?forum-edittopic-'.$tid.'-2" method="POST" class="form-horizontal" role="form">';
      echo '<i class="fa fa-quote-right"></i> <b>Begr&uuml;ndung an den Ersteller (freiwillig)</b><br><br><textarea rows="3" class="form-control" placeholder="Grund Angabe" name="reason"></textarea>';
      echo '<br><br><input class="btn btn-danger" type="submit" value="'.$lang['delete'].'" name="sub">  <a class="btn btn-primary" href="javascript:history.back(1)">&laquo; zur&uuml;ck</a>';
      echo '</form>';
    } else {
  ...
  case 3 : # move topic in another forum
		if ( empty ( $_POST['sub'] ) OR $_POST['nfid'] == 'cat' ) {
			...
			echo '<input type="hidden" name="afid" value="'.$fid.'"><i class="fa fa-bars"></i> <b>neues Forum ausw&auml;hlen</b><br><br>';
      ...
      echo '</select><br><br><input type="checkbox" name="alertautor" value="yes" /> Den Autor &uuml;ber das verschieben informieren?<br><br><input type="submit" class="btn btn-primary" value="Verschieben" name="sub"> <a class="btn btn-primary" href="javascript:history.back(1)">&laquo; zur&uuml;ck</a></form>';
    } else {
